...
try {
    ...
    if (empty($company_id) || empty($company_name) || empty($address)) {
    ...
    $stmt->execute([
        ...
        $phone_number, $email, $representative_name, $decision_maker_1, $decision_maker_2,
        $representative_income, $representative_contact, $employee_count, $capital, $revenue,
        $teikoku, $tosho, $memo, $company_id
    ]);

    header('Location: companies_list.php?status=success&message=顧客企業が更新されました');
    exit;
} catch (Exception $e) {
    header('Location: edit_companies.php?company_id=' . urlencode($_POST['company_id']) . '&status=error&message=' . urlencode($e->getMessage()));
    exit;
}
?>