...
try {
    ...
    if (empty($provider_name) || empty($address)) throw new Exception('会社名と住所は必須です');
    ...
    $stmt = $conn->prepare(
        "INSERT INTO lease_providers (provider_name, business_registration_number, industry_type, address, postal_code, phone_number, email) 
         VALUES (?, ?, ?, ?, ?, ?, ?)"
    );
    $stmt->execute([$provider_name, $business_registration_number, $industry_type, $address, $postal_code, $phone_number, $email]);

    header('Location: lease_providers_list.php?status=success&message=リース会社が追加されました');
    exit;
} catch (Exception $e) {
    header('Location: add_lease_providers.php?status=error&message=' . urlencode($e->getMessage()));
    exit;
}
?>