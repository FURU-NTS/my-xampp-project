...
try {
    ...
    if ($negotiation_status && !in_array($negotiation_status, ['進行中', '与信怪しい', '工事前再説', '工事後再説', '工事前キャンセル', '工事後キャンセル', '書換完了'])) throw new Exception('無効な商談ステータスです');
    if ($construction_status && !in_array($construction_status, ['待ち', '与信待ち', '残あり', '完了', '回収待ち'])) throw new Exception('無効な工事ステータスです');
    ...
    file_put_contents('C:\xampp_new\htdocs\LeaseAndMaintenanceDB\debug.log', 
        "Negotiation Status before insert: " . $negotiation_status . "\n", FILE_APPEND);
    ...
    $stmt = $conn->prepare(
        "INSERT INTO orders (
            ...
            seal_certificate_status, sales_rep_id, sales_rep_id_2, sales_rep_id_3, sales_rep_id_4, 
